...
#define REGBIT_NETWORK_FUNCTION_DIR     "reg8[22]"
#define REGBIT_NETWORK_FUNCTION_EGRESS_LOOPBACK    "reg8[23]"
#define REG_NETWORK_FUNCTION_GROUP_ID "reg5[0..7]"
...
static struct ovn_port *
ovn_port_find_child(const struct hmap *ports, const struct ovn_datapath *od,
                    const char *name)
{
    for (size_t i = 0; i < od->nbs->n_ports; i++) {
        const struct nbrec_logical_switch_port *nbsp = od->nbs->ports[i];
        if (!strcmp(nbsp->name, name) ||
            (nbsp->parent_name && !strcmp(nbsp->parent_name, name))) {
            return ovn_port_find(ports, nbsp->name);
        }
    }
    return NULL;
}
...
ovn_nf_svc_create(struct ovsdb_idl_txn *ovnsb_txn,
                  ...
                  struct hmap *ic_learned_monitors_map,
             ...
             const struct sbrec_acl_id_table *sbrec_acl_id_table)
{
    ...
    if (!strcmp(acl->action, "allow")
        ...
        if (acl->network_function_group) {
            uint8_t nfg_id = acl->network_function_group->id;
            VLOG_DBG("Allow ACL match %s with network_function_group %s",
                     acl->match, acl->network_function_group->name);
            ds_put_format(actions, REGBIT_NETWORK_FUNCTION_ENABLED" = 1; "
                          REGBIT_NETWORK_FUNCTION_DIR" = 1; "
                            ...
                            const char *svc_monitor_ip4_dst)
{
    ...
    for (int i = 0; i < nfg->n_network_function; i++) {
        ...
        if (nf->health_check == NULL) {
            ...
            //TODO: we pass NULL instead of ic_learned_svc_monitors_map
            struct service_monitor_info *mon_info =
            get_service_mon(svc_monitor_map, NULL, svc_monitor_ip4_dst,
                          ...
                          struct lflow_ref *lflow_ref, bool ingress)
{
    ...
    struct ovn_port *output_port = NULL;
    ...
    nf = network_function_get_active(nfg);
    if (nf == NULL) {
     ...
     * ports. */
    input_port = ovn_port_find_child(ports, od, nf->inport->name);
    output_port = ovn_port_find_child(ports, od, nf->outport->name);
     ...
     */
    build_lswitch_stateful_nf(output_port, lflows, &action, &match, lflow_ref);
    build_lswitch_stateful_nf(input_port, lflows, &action, &match, lflow_ref);
                       ...
                       struct lflow_ref *lflow_ref)
{
     ...
     * LS. Drop such packets. This behaviour can be made configurable in future
     */
    ...
    for (size_t i = 0; i < od->nbs->n_acls; i++) {
        acl = od->nbs->acls[i];
        if (acl->network_function_group) {
            ingress = !strcmp(acl->direction, "from-lport") ? true :false;
            nfg_bitmap = ingress ? nfg_ingress_bitmap : nfg_egress_bitmap;
            nfg_id = acl->network_function_group->id;
            if (bitmap_is_set(nfg_bitmap, nfg_id)) {
              VLOG_DBG("Same NF %d used again - skipping", nfg_id);
              continue;
            }
            nfg_bitmap = bitmap_set1(nfg_bitmap, nfg_id);
            VLOG_DBG("Adding %s network_function to switch",
                     ingress == true ? "ingress" : "egress");
            consider_network_function(lflows, od, acl->network_function_group,
                                      ports, lflow_ref, ingress);
        }
    }

    int bitmap_n_bytes = MAX_OVN_NETWORK_FUNCTION_GROUP_IDS / 8;
    memset(nfg_ingress_bitmap, 0x00, bitmap_n_bytes);
    memset(nfg_egress_bitmap, 0x00, bitmap_n_bytes);
    ...
    if (ls_pg) {
        ...
        HMAP_FOR_EACH (ls_pg_rec, key_node, &ls_pg->nb_pgs) {
            for (size_t i = 0; i < ls_pg_rec->nb_pg->n_acls; i++) {
                acl = ls_pg_rec->nb_pg->acls[i];
                if (acl->network_function_group) {
                    ingress = !strcmp(acl->direction, "from-lport") ? true
                                                                    : false;
                    nfg_bitmap
                        = ingress ? nfg_ingress_bitmap : nfg_egress_bitmap;
                    nfg_id = acl->network_function_group->id;
                    if (bitmap_is_set(nfg_bitmap, nfg_id)) {
                      VLOG_INFO("Same NF %d used again - skipping", nfg_id);
                      continue;
                    }
                    nfg_bitmap = bitmap_set1(nfg_bitmap, nfg_id);
                    VLOG_DBG("Adding %s network_function to port-group",
                             ingress == true ? "ingress" : "egress");
...
static void
allocate_network_function_group_ids(struct northd_input *input_data)
{
    const struct nbrec_network_function_group *nfg;
    long unsigned int *allocated_ids
        = bitmap_allocate(MAX_OVN_NETWORK_FUNCTION_GROUP_IDS);
    size_t id;

    bitmap_set1(allocated_ids, 0); /* skip 0 */

    NBREC_NETWORK_FUNCTION_GROUP_TABLE_FOR_EACH (
        nfg, input_data->nbrec_network_function_group_table) {
        if (nfg->id) {
            bitmap_set1(allocated_ids, nfg->id);
        }
    }
    NBREC_NETWORK_FUNCTION_GROUP_TABLE_FOR_EACH (
        nfg, input_data->nbrec_network_function_group_table) {
        if (!nfg->id) {
            id = bitmap_scan(allocated_ids, 0, 1,
                             MAX_OVN_NETWORK_FUNCTION_GROUP_IDS);
            if (id == MAX_OVN_NETWORK_FUNCTION_GROUP_IDS) {
                static struct vlog_rate_limit rl = VLOG_RATE_LIMIT_INIT(1, 1);
                VLOG_ERR_RL(&rl, "out of ids for logical port "
                            "network_function_group %s", nfg->name);
                bitmap_free(allocated_ids);
                return;
            }
            bitmap_set1(allocated_ids, id);
            nbrec_network_function_group_set_id(nfg, id);
        }
    }
    bitmap_free(allocated_ids);
}
                                        ...
                                        struct lswitch_flow_build_info *lsi)
{
    ...
    build_network_function(od, lsi->lflows, lsi->ls_ports, lsi->ls_port_groups,
                           NULL);
                                ...
                                struct lflow_table *lflows)
{
    ...
    HMAPX_FOR_EACH (hmapx_node, &trk_data->crupdated) {
        ...
        build_network_function(od, lflows, lflow_input->ls_ports,
                               lflow_input->ls_port_groups,
                               ls_stateful_rec->lflow_ref);
             ...
             struct ovsdb_idl_txn *ovnsb_txn)
{
    ...
    build_lb_port_related_data(&data->lr_datapaths,
                               ...
                               &data->lb_group_datapaths_map);
    build_svcs(ovnsb_txn, input_data->sbrec_service_monitor_by_learned_type,
               input_data->svc_monitor_mac,
    ...
    cleanup_stale_fdb_entries(input_data->sbrec_fdb_table,
                              &data->ls_datapaths.datapaths);
    allocate_network_function_group_ids(input_data);
