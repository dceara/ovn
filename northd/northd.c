...
static struct ovn_port *
ovn_port_find_child(const struct hmap *ports, struct ovn_datapath *od,
                    const char *name)
{
           ...
           const struct sbrec_acl_id_table *sbrec_acl_id_table)
{
     ...
     * that ACL will allow the matching packets packets to proceed
               ...
               struct lflow_ref *lflow_ref, bool ingress)
{
     ...
     */
    build_lswitch_stateful_nf(output_port, lflows, &action, &match);
    build_lswitch_stateful_nf(input_port, lflows, &action, &match);
...
static void
build_network_function(
            struct ovn_datapath *od, struct lflow_table *lflows,
            const struct hmap *ports, const struct ls_port_group_table *ls_pgs,
            struct lflow_ref *lflow_ref)
{
    bool ingress;
