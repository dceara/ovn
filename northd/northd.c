                   ...
                   const struct sbrec_load_balancer *slb)
{
    ...
    if (!smap_equal(&slb->vips, &lb->nlb->vips)) {
        return true;
    }

    if (!slb->protocol || strcmp(slb->protocol, lb->nlb->protocol)) {
        return true;
    }
    ...
    if (strcmp(smap_get_def(&slb->options, "hairpin_snat_ip", ""),
               smap_get_def(&lb->nlb->options, "hairpin_snat_ip", ""))) {
        return true;
    }

    if (slb->n_datapaths != lb->n_nb_ls) {
        return true;
    }
