                  ...
                  const struct lr_stateful_record *lr_sful_rec)
{
    ...
    if (!op || !op->nbrp || !op->od || !op->od->nbr
        || (!op->od->nbr->n_nat && (!lr_sful_rec
                                    || !lr_sful_rec->has_lb_vip))
                            ...
                            struct lflow_ref *lflow_ref)
{
    ...
    ovs_assert(op->od == lr_sful_rec->od);
    ...
    for (size_t i = 0; i < lr_sful_rec->lrnat_rec->n_nat_entries; i++) {
        struct ovn_nat *nat_entry = &lr_sful_rec->lrnat_rec->nat_entries[i];
        ...
        if (nat_entry_is_v6(nat_entry)) {
            if (!sset_contains(&lr_sful_rec->lb_ips->ips_v6,
                               nat->external_ip)) {
                build_lswitch_rport_arp_req_flow(
                            ...
                            struct ds *actions, struct lflow_ref *lflow_ref)
{
    ...
    ovs_assert(op->peer->od == lr_sful_rec->od);
    ...
    for (size_t i = 0; i < lr_sful_rec->lrnat_rec->n_nat_entries; i++) {
        const struct ovn_nat *nat = &lr_sful_rec->lrnat_rec->nat_entries[i];
                               ...
                               struct lflow_ref *lflow_ref)
{
    ...
    BITMAP_FOR_EACH_1 (index, bitmap_len, lb_dps->nb_lr_map) {
        ...
        const struct lr_stateful_record *lr_sful_rec =
            lr_stateful_table_find_by_index(lr_sful_table, od->index);
        ovs_assert(lr_sful_rec);

        const struct lr_nat_record *lrnat_rec = lr_sful_rec->lrnat_rec;
        if (lb->skip_snat) {
    ...
    if (op->lrp_networks.n_ipv4_addrs) {
        for (size_t i = 0; i < op->lrp_networks.n_ipv4_addrs; i++) {
            ...
            bool router_ip_in_snat_ips =
                !!shash_find(&lr_sful_rec->lrnat_rec->snat_ips, ip);
            bool router_ip_in_lb_ips =
                !!sset_find(&lr_sful_rec->lb_ips->ips_v4, ip);
    ...
    if (op->lrp_networks.n_ipv6_addrs) {
        ...
        for (size_t i = 0; i < op->lrp_networks.n_ipv6_addrs; i++) {
            ...
            bool router_ip_in_snat_ips =
                !!shash_find(&lr_sful_rec->lrnat_rec->snat_ips, ip);
            bool router_ip_in_lb_ips =
                !!sset_find(&lr_sful_rec->lb_ips->ips_v6, ip);
            bool drop_router_ip = (drop_snat_ip == (router_ip_in_snat_ips ||
                                                    router_ip_in_lb_ips));
                            ...
                            struct lflow_ref *lflow_ref)
{
    ...
    for (int i = 0; i < lr_sful_rec->lrnat_rec->n_nat_entries; i++) {
        struct ovn_nat *nat_entry = &lr_sful_rec->lrnat_rec->nat_entries[i];
    ...
    /* Now handle SNAT entries too, one per unique SNAT IP. */
    struct shash_node *snat_snode;
    SHASH_FOR_EACH (snat_snode, &lr_sful_rec->lrnat_rec->snat_ips) {
        struct ovn_snat_ip *snat_ip = snat_snode->data;
    ...
    struct lflow_ref *lflow_ref)
{
     ...
     * related traffic such as an ICMP Port Unreachable through
     * that's generated from a non-listening UDP port.  */
    if (lr_sful_rec->has_lb_vip && features->ct_lb_related) {
        ds_clear(match);
        ...
        if (lsi) {
                    ...
                    bnum += control->pool->size)
            {
                HMAP_FOR_EACH_IN_PARALLEL (op, key_node, bnum,
                    ...
                    build_lbnat_lflows_iterate_by_lrp(op, lsi->lr_sful_table,
                                                      lsi->meter_groups,
                                                      &lsi->match,
                                                      &lsi->actions,
                                                      lsi->lflows,
            ...
            for (bnum = control->id;
                    bnum <= lsi->lr_sful_table->entries.mask;
                    bnum += control->pool->size)
            {
                LR_STATEFUL_TABLE_FOR_EACH_IN_P (lr_sful_rec, bnum,
                                                    lsi->lr_sful_table) {
                    if (stop_parallel_processing()) {
                        return NULL;
                    }
                    build_lr_stateful_flows(lr_sful_rec,
            ...
            for (bnum = control->id;
                    bnum <= lsi->ls_sful_table->entries.mask;
                    bnum += control->pool->size)
            {
                                ...
                                const struct chassis_features *features)
{
    ...
    if (parallelization_state == STATE_USE_PARALLELIZATION) {
        ...
        LR_STATEFUL_TABLE_FOR_EACH (lr_sful_rec, lr_sful_table) {
            build_lr_stateful_flows(lr_sful_rec, lsi.lflows,
                                    lsi.ls_ports, lsi.lr_ports, &lsi.match,
                                    &lsi.actions, lsi.meter_groups,
                                    lsi.features);
        }

        LS_STATEFUL_TABLE_FOR_EACH (ls_stateful_rec, ls_sful_table) {
