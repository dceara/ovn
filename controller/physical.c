    ...
    struct ovn_desired_flow_table *flow_table)
{
    ...
    if (!linked_zone.ct) {
        VLOG_INFO("Linked port zone not found for %s", binding->logical_port);
        return;
    }
    VLOG_INFO("Both port zones found for NF port %s", binding->logical_port);
    put_redirect_overlay_to_source(
                                 binding,
                                 linked_zone.ct,
                                 chassis_tunnels,
                      ...
                      struct ofpbuf *ofpacts_p)
{
    ...
    bool is_nf = (smap_get(&binding->options, "network-function-linked-port") ?
                           true: false);
    if (!ofport) {
