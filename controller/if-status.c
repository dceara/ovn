...
struct if_status_mgr {
    ...
    /* Interface specific seqno to be acked by ofctrl when flows for new interfaces
     * have been installed.
     */
                     ...
                     struct local_binding_data *binding_data)
{
     ...
     */
    HMAPX_FOR_EACH_SAFE (node, node_next, &mgr->ifaces_per_state[OIF_MARK_UP]) {
     ...
     * module.
     */
    HMAPX_FOR_EACH_SAFE (node, node_next, &mgr->ifaces_per_state[OIF_MARK_DOWN]) {
        struct ovs_iface *iface = node->data;
    ...
    HMAPX_FOR_EACH_SAFE (node, node_next, &mgr->ifaces_per_state[OIF_CLAIMED]) {
        struct ovs_iface *iface = node->data;
    ...
    if (new_ifaces) {
        mgr->iface_seqno++;
        ofctrl_seqno_update_create(mgr->iface_seq_type_pb_cfg, mgr->iface_seqno);
                  ...
                  bool sb_readonly, bool ovs_readonly)
{
     ...
     * in OVS.
     */
    HMAPX_FOR_EACH_SAFE (node, node_next, &mgr->ifaces_per_state[OIF_INSTALL_FLOWS]) {
        struct ovs_iface *iface = node->data;
    ...
    if_status_mgr_update_bindings(mgr, binding_data, sb_readonly, ovs_readonly);
}

static struct ovs_iface *
ovs_iface_create(struct if_status_mgr *mgr, const char *iface_id, enum if_state state)
{
...
static void
ovs_iface_set_state(struct if_status_mgr *mgr, struct ovs_iface *iface, enum if_state state)
{
