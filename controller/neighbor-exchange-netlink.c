...
struct ne_msg_handle_data {
    ...
    /* Filters only relevant neighbor entries to be learned. */
    ne_nl_neigh_filter *neigh_filter;
 ...
 * 'if_index'.
 *
 * Optionally, 'neigh_filter' can be passed as argument to further filter
 * what entries should be learned.
 *
                 ...
                 const struct ndmsg *nd, struct ne_table_msg *change)
{
    ...
    if (parsed) {
        ...
        change->nd.if_index = nd->ndm_ifindex;
        change->nd.family = nd->ndm_family;
        change->nd.state = nd->ndm_state;
        change->nd.flags = nd->ndm_flags;
        change->nd.type = nd->ndm_type;
...
handle_ne_msg(const struct ne_table_msg *msg, void *data)
{
    ...
    if (!ne_is_ovn_owned(nd)) {
        ...
        /* Learn the non-OVN entry if it matches the filter or if no
         * filtering was requested. */
        if (!handle_data->neigh_filter || handle_data->neigh_filter(nd)) {
