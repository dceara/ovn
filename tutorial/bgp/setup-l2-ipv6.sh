...
# configure BGP IPs
podman exec $h1 ip -6 a a dev eth0 20::1/8
podman exec $h2 ip -6 a a dev eth0 20::2/8
podman exec $h3 ip -6 a a dev eth0 20::3/8

podman exec $h1 ping -c 1 20::2
podman exec $h1 ping -c 1 20::3
...
echo "configure
  router bgp 65000
    ...
    neighbor 20::2 remote-as 65000
    neighbor 20::3 remote-as 65000
    !
...
for vni in 10 20; do
    ...
    podman exec $h1 ip link add vxlan-$vni type vxlan id $vni dstport 4789 local 20::1 nolearning
...
for vni in 10 20; do
    ...
    # Add VXLAN VTEP for the VNI.
    podman exec $h2 ip link add vxlan-$vni type vxlan id $vni dstport 4789 local 20::2 nolearning
...
for vni in 10 20; do
    ...
    # Add VXLAN VTEP for the VNI.
    podman exec $h3 ip link add vxlan-$vni type vxlan id $vni dstport 4789 local 20::3 nolearning
