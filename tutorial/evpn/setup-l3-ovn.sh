...
for vni in 10 20; do
    ...
    podman exec $h3 ip link set br-$vni type bridge vlan_filtering 0
    ...
    podman exec $h3 ip link set vxlan-$vni master br-$vni addrgenmode none
    podman exec $h3 ip link set vxlan-$vni type bridge_slave neigh_suppress on learning off
