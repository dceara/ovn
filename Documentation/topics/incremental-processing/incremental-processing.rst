...
- **Engine Node:** Represents a unit of data processing. Each node maintains its
  own data, which persists across main loop iterations.
...
- **Maintainability:** By providing a structured way to implement complex logic.
...
For complex graphs, it can be useful to view only a subset of the nodes. You can
provide a specific node name as an argument to dump the graph leading up to that
node (including the node itself and its transitive dependencies).
...
You can convert the DOT output to an image (e.g., PNG) using the `dot` utility
from Graphviz:
...
The engine core is implemented in `lib/inc-proc-eng.c` and `lib/inc-proc-eng.h`.
