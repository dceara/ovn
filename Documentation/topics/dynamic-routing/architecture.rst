...
When a logical router has ``dynamic-routing`` set to ``true`` in its
``other_config``, ``ovn-northd`` examines the router configuration and
populates the ``Advertised_Route`` table in the OVN Southbound database.
The types of routes that are advertised depend on the
``dynamic-routing-redistribute`` option, which accepts a comma-separated
...
This information is processed entirely in memory by each
``ovn-controller`` instance and is not written to the SB database.
