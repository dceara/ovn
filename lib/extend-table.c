...
void
ovn_extend_table_reinit(struct ovn_extend_table *table,
                        uint32_t last_usable_id)
{
    if (last_usable_id != table->last_usable_id) {
        table->last_usable_id = last_usable_id;
        id_pool_destroy(table->table_ids);
        table->table_ids = id_pool_create(1, last_usable_id);
    }
}
